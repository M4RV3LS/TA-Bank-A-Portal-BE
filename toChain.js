...
if (
  ...
  console.error(
    "[Bank A - toChain.js config] ERROR: Missing one or more required .env variables (PRIVATE_KEY, KYC_REGISTRY_ADDRESS, CUSTOMER_PORTAL_INTERNAL_API_URL, RPC_URL)."
  );
  process.exit(1);
}
...
function computeHash(input, label = "Input") {
  if (typeof input !== "string") {
    console.warn(
      `[Bank A - toChain - computeHash] ${label} is not a string for hashing, type: ${typeof input}. Hashing an empty string instead.`
    );
    return ethers.keccak256(ethers.toUtf8Bytes(""));
  }
  // console.log(`[Bank A - toChain - computeHash] Hashing ${label} (first 50 chars): ${input.substring(0,50)}... (length: ${input.length})`);
...
async function updateRequestStatus(requestId, newStatus, note = null) {
  try {
    ...
    console.log(
      `[Bank A - toChain - updateRequestStatus] Request ${requestId} status updated to ${newStatus}` +
        (note ? ` with note: "${note}"` : "")
    );
  } catch (dbErr) {
    console.error(
      `[Bank A - toChain - updateRequestStatus] DB Error updating request ${requestId} to ${newStatus}:`,
      dbErr
    );
  }
}
...
async function syncBundleWithCustomerPortal(
  ...
  console.log(
    `[Bank A - toChain - syncBundleWithCP] Calling CP (${customerPortalRotateKeyUrl}) for user ${userId}.`
  );
  ...
  try {
    ...
    const cpResponse = await fetchModule.default(customerPortalRotateKeyUrl, {
      ...
      body: JSON.stringify({
        ...
        callingBankId: callingBankIdentifier, // e.g., "BANK_A" from process.env.THIS_BANK_IDENTIFIER
      }),
    });
    ...
    if (!cpResponse.ok) {
      console.error(
        `[Bank A - toChain - syncBundleWithCP] CP call failed for user ${userId}. Status: ${cpResponse.status}, Body: ${responseText}`
      );
      throw new Error(
    ...
    try {
      ...
      console.error(
        `[Bank A - toChain - syncBundleWithCP] Failed to parse JSON from CP for user ${userId}. Raw: ${responseText}`,
        parseError
      );
      throw new Error(
    ...
    if (
      ...
      console.error(
        `[Bank A - toChain - syncBundleWithCP] CP response missing required fields for user ${userId}. Response:`,
        cpResponseJson
      );
      throw new Error(
    ...
    console.log(
      `[Bank A - toChain - syncBundleWithCP] Received from CP for user ${userId}:`
    );
    ...
    console.log(
      `[Bank A - toChain - syncBundleWithCP] Successfully updated banka_portal.user_profiles for user_id ${userId} with bundle from CP.`
    );
    ...
    console.error(
      `[Bank A - toChain - syncBundleWithCP] Error during sync for user ${userId}:`,
      err
    );
    throw err;
  }
...
async function sendToChain(requestRow) {
  ...
  console.log(
    `[Bank A - sendToChain] Processing request_id: ${request_id}, client_id: ${client_id}, status_request: '${status_request}'`
  );
  ...
  console.log(
    `[Bank A - sendToChain] Data for Hashing/Bundling - ClientID ${client_id}:`
  );
  console.log(
  ...
  console.log(
    `[Bank A - sendToChain] For client ${client_id}: HashKTP to be sent: ${hashKtp}, HashKYC to be sent: ${hashKyc}`
  );
  ...
  try {
    ...
    try {
      ...
      console.warn(
        `[Bank A - sendToChain] kyc.getVersionCount for client ${client_id} failed or new client. Assuming 0. Error: ${e.message}`
      );
      onChainVersionCount = 0;
    }

    console.log(
      `[Bank A - sendToChain] Client ${client_id}: DB status_request '${status_request}', On-chain version count: ${onChainVersionCount}.`
    );
    ...
    if (status_request === "new") {
      if (onChainVersionCount > 0) {
        ...
        console.error(`[Bank A - sendToChain] ${errorMsg}`);
      ...
      if (onChainVersionCount === 0) {
        console.warn(
          `[Bank A - sendToChain] Client ${client_id} (request ${request_id}) is 'update' but not on-chain. Treating as initial submission (1.0 ETH).`
        );
        ethAmountString = "1.0";
      } else {
        ethAmountString = "0.1";
    ...
    console.log(
      `[Bank A - sendToChain] Determined ETH amount for request ${request_id}: ${ethAmountString} ETH.`
    );
    ...
    console.log(
      `[Bank A - sendToChain] Successfully executed CP bundle sync for client ${client_id} after on-chain update.`
    );

    await updateRequestStatus(request_id, "success");
    console.log(
      `[Bank A - sendToChain] Successfully processed KYC for request ${request_id} (client ${client_id}) on-chain and synced with CP. Tx: ${onChainResult.txHash}, Version: ${onChainResult.version}`
    );
    ...
    console.error(
      `[Bank A - sendToChain] Error processing request ${request_id} (client ${client_id}):`,
      err.message,
      err.stack
    );
    ...
    if (onChainResult && onChainResult.txHash) {
      console.error(
        `[Bank A - sendToChain] CRITICAL: On-chain TX for request ${request_id} (client ${client_id}) SUCCEEDED (Tx: ${onChainResult.txHash}), but bundle sync FAILED: ${err.message}.`
      );
      await updateRequestStatus(
        request_id,
        "success",
