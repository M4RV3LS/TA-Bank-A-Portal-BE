...
// --- Configuration ---
// Ensure these are correctly pointing to Bank A's DB
const dbConfig = {
  host: process.env.DB_HOST || "localhost",
  user: process.env.DB_USER || "root",
  password: process.env.DB_PASSWORD || "",
  database: process.env.DB_NAME || "banka_portal", // Should be banka_portal
};
...
async function main() {
  try {
    console.log(`Connecting to Bank A's database: ${dbConfig.database}...`);
    await util.promisify(connection.connect).bind(connection)();
    console.log("Connected to Bank A's database.");
    ...
    for (const client of clients) {
      ...
      try {
        ...
        console.log(
          `  Successfully populated/updated local encrypted_bundle for user_id ${userId} in banka_portal.user_profiles.`
        );
      } catch (dbUpsertError) {
        console.error(
