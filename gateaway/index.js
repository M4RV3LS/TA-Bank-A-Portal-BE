// bank-portal/bank-a/backend/gateaway/index.js
const express = require("express");
const ethers = require("ethers");
const jwt = require("jsonwebtoken");
const connection = require("../dbConnection"); // Bank A's DB Connection
...
if (!MY_BANK_ETHEREUM_ADDRESS) {
  console.error(
    "[Bank A - Gateway FATAL] MY_BANK_ETHEREUM_ADDRESS is not configured. This is required."
  );
  process.exit(1);
}
if (!KNOWN_BANK_GATEWAYS || Object.keys(KNOWN_BANK_GATEWAYS).length === 0) {
  console.warn(
    "[Bank A - Gateway WARN] KNOWN_BANK_GATEWAYS is not configured or is empty. Fetching from Home Bank will likely fail."
  );
}
...
app.get("/kyc-files/:clientId", authMw, async (req, res) => {
  ...
  console.log(
    `[Bank A - Gateway /kyc-files] Request for clientId: ${clientId} from bank: ${originalRequesterAddress}`
  );

  if (!originalRequesterAddress) {
    console.error(
      "[Bank A - Gateway /kyc-files] Original requester address missing from auth badge."
    );
    return res
      .status(401)
  ...
  try {
    console.log(
      `[Bank A - Gateway /kyc-files] On-chain check for clientId: ${clientId}, as gateway: ${myGatewayIdentityAddress}. Original requester: ${originalRequesterAddress}`
    );
    ...
    try {
      if (typeof contract.getVersionCount !== "function") {
        console.error(
          "[Bank A - Gateway /kyc-files] Smart contract 'getVersionCount' not found."
        );
      ...
      console.log(
        `[Bank A - Gateway /kyc-files] No KYC record for clientId ${clientId} (getVersionCount error):`,
        e.message
      );
    }
    ...
    if (versionCount > 0n) {
      if (typeof contract.getKycByVersion !== "function") {
        console.error(
          "[Bank A - Gateway /kyc-files] Smart contract 'getKycByVersion' not found."
        );
        return res.status(500).json({
          error:
      ...
      if (homeBankAddressOnChain === myGatewayIdentityAddress) {
        ...
        console.log(
          `[Bank A - Gateway /kyc-files] Access Check: This gateway (${myGatewayIdentityAddress}) IS Home Bank for clientId ${clientId}.`
        );
      } else {
        if (typeof contract.getParticipatingBanks !== "function") {
          console.error(
            "[Bank A - Gateway /kyc-files] Smart contract 'getParticipatingBanks' not found."
          );
          return res.status(500).json({
            error:
        ...
        if (
          ...
          // The *requesting bank* (e.g. Bank B) must be a participant to get data via *this* gateway (Bank A)
          // AND this gateway (Bank A) must also be permitted if it's not the home bank but an intermediary (though current logic focuses on home bank or this gateway being participant for originalRequester)
          // For now, the main check is if this gateway itself is permitted to serve data FOR the original requester.
          // If Bank A is Home Bank, originalRequesterAddress doesn't strictly need to be a participant to ask Bank A for data it owns.
          // If Bank A is NOT Home Bank, but IS a participant, it might be fetching on behalf of originalRequesterAddress if originalRequesterAddress is also a participant.
          ...
          // This is correct: if Bank A is a participant, it's allowed to access the data (potentially to serve to another participant).
          if (
            participatingBanks
              ?.map((addr) => addr.toLowerCase())
            ...
            isPermitted = true;
            console.log(
              `[Bank A - Gateway /kyc-files] Access Check: This gateway (${myGatewayIdentityAddress}) IS a Participating Bank for clientId ${clientId}. Home Bank is ${homeBankAddressOnChain}.`
            );
          }
        }
      }
      ...
      console.log(
        `[Bank A - Gateway /kyc-files] Access Check: No KYC records found for clientId ${clientId}.`
      );
    }

    if (!isPermitted) {
      console.warn(
        `[Bank A - Gateway /kyc-files] Access DENIED for gateway ${myGatewayIdentityAddress} to serve clientId ${clientId} data to ${originalRequesterAddress}.`
      );
    ...
    if (homeBankAddressOnChain === myGatewayIdentityAddress) {
      ...
      console.log(
        `[Bank A - Gateway /kyc-files] Is Home Bank. Fetching bundle for clientId: ${clientId} from own DB.`
      );
      connection.query(ENCRYPTED_BUNDLE_SQL, [clientId], (dbErr, rows) => {
        if (dbErr) {
          console.error(
            "[Bank A - Gateway /kyc-files] DB Error fetching bundle:",
            dbErr
          );
          if (!res.headersSent) {
            return res
        ...
        if (!rows.length || !rows[0].encrypted_bundle) {
          console.warn(
            `[Bank A - Gateway /kyc-files] No bundle found in own DB for clientId: ${clientId}`
          );
          if (!res.headersSent) {
        ...
        console.log(
          `[Bank A - Gateway /kyc-files] Serving bundle for clientId: ${clientId} from own DB.`
        );
        ...
        console.log(
          `[Bank A - Gateway /kyc-files] Successfully served bundle from own DB for clientId: ${clientId}. Token issued to ${originalRequesterAddress}.`
        );
      });
    } else if (
      ...
      // This specific Bank A gateway will act as a client to the actual Home Bank's gateway.
      const actualHomeBankGatewayUrl =
        KNOWN_BANK_GATEWAYS[homeBankAddressOnChain];
      console.log(
        `[Bank A - Gateway /kyc-files] Is Participating Bank. Proxying request for clientId: ${clientId} from actual Home Bank ${homeBankAddressOnChain} at ${actualHomeBankGatewayUrl}.`
      );

      const interBankTokenPayload = {
        address: myGatewayIdentityAddress, // This gateway (Bank A) is the requester to the Home Bank Gateway
      ...
      try {
        ...
        if (!actualHomeBankResponse.ok) {
          ...
          console.error(
            `[Bank A - Gateway /kyc-files] Error from Actual Home Bank Gateway (${actualHomeBankGatewayUrl}): ${actualHomeBankResponse.status} - ${errorBody}`
          );
          if (!res.headersSent) {
            return res.status(actualHomeBankResponse.status).json({
        ...
        console.log(
          `[Bank A - Gateway /kyc-files] Successfully fetched bundle from actual Home Bank for clientId: ${clientId}.`
        );
        ...
        console.error(
          `[Bank A - Gateway /kyc-files] Network error fetching from Actual Home Bank Gateway (${actualHomeBankGatewayUrl}):`,
          fetchError
        );
        if (!res.headersSent) {
      ...
      console.error(
        `[Bank A - Gateway /kyc-files] Cannot serve bundle: Home Bank is ${homeBankAddressOnChain}, but its gateway URL is not in KNOWN_BANK_GATEWAYS or Home Bank not identified on-chain.`
      );
      if (!res.headersSent) {
        return res.status(500).json({
          error:
    ...
    console.error(
      `[Bank A - Gateway /kyc-files] Critical error during on-chain access check for clientId ${clientId}:`,
      onChainError
    );
    if (!res.headersSent) {
      return res.status(500).json({
...
app.listen(PORT, () => {
  console.log(`Bank A Gateway listening on http://localhost:${PORT}`);
  connection.getConnection((err, conn) => {
    if (err) {
      console.error(
        "❌ [Bank A - Gateway] MySQL Pool Error on startup:",
        err.message
      );
    } else {
      console.log(
        "✅ [Bank A - Gateway] Successfully connected to MySQL database pool."
      );
      conn.release();
    }
  });
