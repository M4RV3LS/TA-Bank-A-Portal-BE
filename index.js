...
const { sendToChain } = require("./toChain"); // Ensure this uses BANK A's signer/config
...
const allowedOrigins = [
  ...
  "http://localhost:3001", // Bank A Frontend
  "http://localhost:3003", // BANK A Frontend (assuming it runs on 3003)
...
try {
  ...
  console.log(
    `[ETH Setup BANK A] Connected to contract ${process.env.KYC_REGISTRY_ADDRESS} with signer ${signer.address}`
  );
} catch (ethSetupError) {
  console.error(
...
// Pay endpoint
// In bank-a/backend/index.js (and bank-a/backend/index.js)
...
// Fetch and Verify Reuse KYC Data (Endpoint called by BANK A's frontend)
app.post(
...
const PORT = process.env.PORT || 4000; // For Bank a, should be 4000
app.listen(PORT, () =>
  console.log(`🚀 BANK A backend running on http://localhost:${PORT}`)
);
